...
- Фаза 4 (редактирование и UX): частично — 4.1 ✅ (geoman: создание/перемещение/правка геометрии/удаление), 4.2 🟡 (базовые формы атрибутов из attrs_schema), 4.3 🟡 (attrs_schema в admin как JSON-textarea), 4.4 ⬜ (CRUD связей объектов не начат)
...
1. Завершить фазу 4: 4.3 — структурный редактор attrs_schema в admin; 4.2 — расширить типы полей форм; 4.4 — CRUD связей объектов (эндпоинты `/relations` + отрисовка на карте).
