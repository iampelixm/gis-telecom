...
Отрицательные/риски:
- бандл Martin (36 МБ) собран вручную и закоммичен как `infra/tiles/martin.tar.gz` — Dockerfile распаковывает при сборке (воспроизводимая сборка из исходников описана в roadmap/фаза 5);
...
## Доработки после принятия

- `--cache-expiry 5s` у Martin — быстрая инвалидация тайлов после редактирования объектов (фаза 4.1).
