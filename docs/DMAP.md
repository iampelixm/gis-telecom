...
- `GET /geo/suggest?query=…` — автоподсказки адреса (в форме дома и в поиске на карте).
- `GET /geo/reverse?lat=&lon=` — «Адрес по точке» на карте; возвращает полные данные: `address`, `fiasId`, `kladrId`, `regionWithType`, `cityWithType`, `settlementWithType`, `streetWithType`, `houseFull`, `floors`, `apartments`, `postalCode`, `qcGeo`, `lat`, `lon`.
- `GET /geo/forward?address=…`, `GET /geo/company?query=…`.
- Кэш ответов в Redis (TTL 7 дней), провайдер `mock | dadata` без изменения фронта.
- Инструменты на карте (`web`): строка поиска адреса по центру сверху (подсказки → перелёт на точку с маркером) и кнопка «Адрес по точке» (режим клика → панель с полной информацией об адресе).
...
### 4.5 Адреса и геокодирование (карта)
- **Поиск адреса** — строка поиска по центру сверху карты; ввод от 3 символов → подсказки Dadata; выбор → перелёт к адресу и красный маркер.
- **Адрес по точке** — кнопка «Адрес по точке» (рядом со строкой поиска) включает режим клика: кликните по карте → панель с полной информацией об адресе (регион, город, улица, дом, индекс, ФИАС, КЛАДР, этажность, координаты) и маркер. Режим разовый — после клика выключается сам.
- **Закрыть/сброс** — клавиша `Esc` (закрывает подсказки, выключает режим клика или панель адреса).
